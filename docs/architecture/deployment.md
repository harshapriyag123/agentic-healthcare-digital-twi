...
flowchart TB
  ...
  Vercel --> Replay[Deterministic baseline replay]
...
`render.yaml`, `vercel.json`, and the production `Dockerfile` implement this reference architecture. Direct baseline lookup uses bounded process memory; serverless counterfactual requests carry a schema-bounded input for deterministic replay when process affinity is unavailable. Vercel performs SPA deep-link rewrites and adds browser security headers. Render terminates HTTPS and runs UID 10001. Provider credentials, OTLP headers, and deployment actions remain outside Git.
