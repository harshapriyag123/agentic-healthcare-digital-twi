...
class AgentOrchestrator:
    def __init__(self, agents: list[Agent] | None = None) -> None:
        self.agents = agents if agents is not None else [DetectionAgent(), SecurityAgent(), PlanningAgent()]
    ...
    def run(self, context: dict, simulation_id: str, scenario_name: str) -> list[AgentDecision]:
        ...
        for sequence, agent in enumerate(self.agents, start=1):
            if stop_after_detection_failure:
                records.append(self._skipped_record(agent, sequence, "Skipped because detection did not complete safely."))
                continue

            started_at = _iso_now()
            started = perf_counter()
            stage = getattr(agent, "stage", "agent-evaluation")
            logger.info("Agent execution started", extra={"simulation_id": simulation_id, "agent_name": agent.name, "agent_stage": stage, "agent_status": "running"})
            with tracer.start_as_current_span("agent.execute") as span:
                ...
                try:
                    ...
                    human_review = decision.action in {"quarantine-and-require-human-review", "activate-regional-load-balancing"}
                    warning = None
                    status = "completed"
                    ...
                    logger.log(log_level, "Agent execution completed", extra={"simulation_id": simulation_id, "agent_name": agent.name, "agent_stage": stage, "agent_action": record.action, "agent_status": status})
                    if human_review:
                        logger.warning("Human review requested", extra={"simulation_id": simulation_id, "agent_name": agent.name, "agent_stage": stage, "agent_action": record.action, "agent_status": status})
                    ...
                    span.set_attributes({"agent.status": "failed", "agent.human_review_required": True})
                    logger.exception("Agent execution failed", extra={"simulation_id": simulation_id, "agent_name": agent.name, "agent_stage": stage, "agent_action": "defer-to-human-review", "agent_status": "failed"})
                    records.append(AgentDecision(
                        agent=agent.name,
                        agent_id=agent.name,
                        agent_name=getattr(agent, "display_name", agent.name),
                        purpose=getattr(agent, "purpose", None),
        ...
        average_confidence = round(sum(record.confidence for record in records if record.status != "skipped") / max(len([record for record in records if record.status != "skipped"]), 1), 3)
        meta_action = "defer-to-human-review" if failures else "coordinate-validated-response" if context.get("regional_risk", 0) >= 0.45 else "maintain-observability"
        records.append(AgentDecision(
            ...
            explanation="The response is deferred to authorized human review because an execution component failed." if failures else "The agent ensemble prioritizes human review and bounded transfers when risk is elevated.",
            evidence_ids=["regional-risk", "telemetry-integrity"],
            human_review_required=bool(failures) or context.get("regional_risk", 0) >= 0.45,
            warning="Incomplete execution prevents a confident recommendation." if failures else None,
            attributes={"scenario.name": scenario_name},
        ))
        return records
