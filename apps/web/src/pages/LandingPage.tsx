...
export function LandingPage() {
    const { hospitals, scenarios, runs, health, catalogState, setSelectedScenario, runSimulation, runState } = useSimulation();
    ...
    return <div className="landing">
        ...
        <section><div className="section-heading"><div><span className="eyebrow">Live application summary</span><h2>Research platform status</h2></div></div><div className="summary-grid"><div><strong>{catalogState === 'loading' ? '—' : hospitals.length}</strong><span>Hospitals represented</span><small>Current synthetic catalog</small></div><div><strong>{catalogState === 'loading' ? '—' : scenarios.length}</strong><span>Available crisis scenarios</span><small>Current scenario catalog</small></div><div><strong>4</strong><span>Decision agents</span><small>3 implemented agents + meta-orchestrator</small></div><div><strong>{runs.length}</strong><span>Simulations run</span><small>Current browser session</small></div><div><strong>{catalogState === 'loading' ? 'Waking…' : health?.status === 'ok' ? 'Healthy' : 'Unavailable'}</strong><span>Backend status</span><small>{catalogState === 'loading' ? 'Vercel serverless cold start may take up to one minute' : 'Live API health check'}</small></div><div><strong>{import.meta.env.VITE_SIGNOZ_DASHBOARD_URL ? 'Configured' : 'Not connected'}</strong><span>Persisted telemetry</span><small>In-app execution evidence remains available</small></div></div></section>
    </div>;
}
