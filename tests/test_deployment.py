...
def test_supported_production_configuration_and_public_metadata():
    settings = Settings(app_env="production", app_version="release-1", cors_allowed_origins="https://demo.example", trusted_hosts="api.example", otel_enabled=False)
    assert settings.cors_origin_list == ["https://demo.example"]
    assert settings.public_metadata == {"environment": "production", "version": "release-1", "service": "geotwin-api"}


def test_unsupported_environment_and_unsafe_production_origins_fail_safely():
    with pytest.raises(ValidationError): Settings(app_env="development")
    with pytest.raises(ValueError):
        assert Settings(app_env="production", cors_allowed_origins="http://localhost:5173").cors_origin_list
    with pytest.raises(ValueError):
...
def test_invalid_otlp_url_is_rejected_without_exposing_configuration():
    with pytest.raises(ValidationError): Settings(otel_exporter_otlp_endpoint="not-a-url")
...
def test_cors_allows_configured_local_origin_and_rejects_unknown_origin():
    allowed = client.options("/api/v1/health", headers={"Origin": "http://localhost:5173", "Access-Control-Request-Method": "GET"})
    rejected = client.options("/api/v1/health", headers={"Origin": "https://evil.example", "Access-Control-Request-Method": "GET"})
...
def test_request_size_limit_returns_clear_413():
    response = client.post("/api/v1/simulations/run", content=b"x"*1_048_577, headers={"Content-Type": "application/json"})
