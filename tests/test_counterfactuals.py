...
def request(scenario_name="flood-grid-cascade", tampering=0.25, missing=0.15):
    return SimulationRequest(
        scenario_name=scenario_name,
        horizon_hours=48,
        demand_multiplier=1.35,
        hazard=HazardInput(
            heat_index=108, flood_severity=0.8, air_quality_index=180, grid_outage_probability=0.85
        ),
        cyber_event=CyberEvent(
            target_hospital_id="HOSP-DFW-002", severity=0.8, telemetry_tampering=tampering
        ),
...
def comparison(simulation_request=None, identifiers=None):
    baseline_request = (simulation_request or request()).model_copy(
    ...
    result = run_counterfactual_comparison(
        CounterfactualRunRequest(
            ...
            interventions=[
                InterventionSelection(intervention_id=item)
                for item in (identifiers or ALL_INTERVENTIONS)
            ],
        )
    )
...
def test_duplicate_unknown_and_missing_baselines_are_rejected():
    ...
    duplicate = {
        "simulation_id": baseline.simulation_id,
        "interventions": [
            {"intervention_id": "network-segmentation"},
            {"intervention_id": "network-segmentation"},
        ],
    }
    ...
    assert (
        client.post(
            ...
            json={
                "simulation_id": baseline.simulation_id,
                "interventions": [{"intervention_id": "unknown"}],
            },
        ).status_code
        == 422
    )
...
def test_each_intervention_flows_through_the_twin_model():
    ...
    assert sum(state.cyber_loss for state in segmented.hospital_states) < sum(
        state.cyber_loss for state in baseline.affected_hospitals
    )
    assert sum(state.dependency_pressure for state in backup.hospital_states) < sum(
        state.dependency_pressure for state in baseline.affected_hospitals
    )
    assert sum(state.effective_capacity for state in surge.hospital_states) > sum(
        state.effective_capacity for state in baseline.affected_hospitals
    )
    ...
    reroute_by_baseline = {
        state.hospital_id: state for state in reroute_baseline.affected_hospitals
    }
    assert rerouted.total_transfer_patients > 0
    assert any(
        state.estimated_demand != reroute_by_baseline[state.hospital_id].estimated_demand
        for state in rerouted.hospital_states
    )
...
def test_combined_effect_is_bounded_deterministic_and_outcomes_are_distinct():
    ...
    first_values = [
        (
            ...
            item.regional_risk_score,
            item.resilience_score,
            item.telemetry_integrity,
            item.hospital_states,
            item.transfer_plan,
        )
    ...
    assert all(
        state.effective_capacity <= 1.1 * hospital.staffed_beds
        for state, hospital in zip(combined.hospital_states, service.HOSPITALS, strict=True)
    )
...
def test_comparison_deltas_are_calculated_from_baseline():
    ...
    assert intervention.absolute_risk_reduction == pytest.approx(
        result.baseline.regional_risk_score - intervention.regional_risk_score, abs=0.001
    )
    assert intervention.resilience_improvement == pytest.approx(
        intervention.resilience_score - result.baseline.resilience_score, abs=0.001
    )
    assert intervention.unserved_demand_reduction == pytest.approx(
        result.baseline.unserved_demand - intervention.unserved_demand, abs=0.01
    )
...
def test_low_trust_prevents_overconfident_recommendation():
    low_trust = request(scenario_name="wildfire-telemetry", tampering=0.95, missing=0.7)
    _, result = comparison(low_trust, ["network-segmentation", "regional-surge-capacity"])
    assert result.baseline.telemetry_integrity < 0.5
