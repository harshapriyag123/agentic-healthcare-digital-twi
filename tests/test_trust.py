...
def run_scenario(identifier: str):
    scenario = next(
        item for item in client.get("/api/v1/scenarios").json() if item["id"] == identifier
    )
...
def test_trust_score_is_bounded_and_visible_contributions_sum():
    ...
    assert (
        abs(
            sum(item["weighted_contribution"] for item in trust["factor_contributions"])
            - trust["trust_score"]
        )
        <= 0.002
    )
...
def test_same_inputs_have_deterministic_trust_and_stable_unique_evidence_ids():
    ...
    def stable(records):
        return [
            {key: value for key, value in item.items() if key not in ignored} for item in records
        ]
...
def test_missing_evidence_reduces_completeness_and_requires_review():
    scenario = next(
        item
        for item in client.get("/api/v1/scenarios").json()
        if item["id"] == "flood-grid-cascade"
    )
    request = deepcopy(scenario["request"])
    request["missing_telemetry_ratio"] = 0.6
    trust = client.post("/api/v1/simulations/run", json=request).json()["trust"]
    assert trust["evidence_completeness"] == 0.4
...
def test_stale_and_conflicting_evidence_reduce_quality_dimensions():
    ...
    stale = evidence_quality_dimensions(
        [base.model_copy(update={"freshness_status": "stale", "integrity_status": "stale"})]
    )
    conflict = evidence_quality_dimensions(
        [base, base.model_copy(update={"evidence_id": "b", "integrity_status": "conflicting"})]
    )
...
def test_flood_has_no_unmodeled_tampering_and_heatwave_has_cyber_evidence():
    ...
    cyber = next(
        item for item in heat["evidence"] if item["evidence_id"] == "tampering-probability"
    )
...
def test_telemetry_verification_improves_modeled_trust_not_hazard():
    ...
    response = client.post(
        ...
        json={
            ...
            "interventions": [{"intervention_id": "telemetry-verification", "parameters": {}}],
        },
    ).json()
    ...
    assert all(
        state["hazard_pressure"] == baseline["hazard_pressure"]
        for state, baseline in zip(
            outcome["hospital_states"], response["baseline"]["hospital_states"], strict=True
        )
    )


def test_empty_evidence_quality_is_cautious():
    assert evidence_quality_dimensions([]) == {
        "reliability": 0,
        "provenance": 0,
        "freshness": 0,
        "consistency": 0,
